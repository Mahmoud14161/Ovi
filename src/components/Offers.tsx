...
function ProductCard({ product, onCheckout }: { product: typeof PRODUCTS[0], onCheckout: OffersProps['onCheckout'] }) {
  ...
  return (
    <div className="bg-white rounded-2xl p-4 sm:p-6 md:p-8 shadow-[0_8px_30px_rgba(0,0,0,0.06)] border border-gray-100 flex flex-col md:flex-row gap-4 sm:gap-8 mb-8 w-full max-w-4xl mx-auto items-center" dir="rtl">
      
      {/* Product Image */}
      <div className="relative w-full md:w-1/3 flex justify-center items-center h-48 sm:h-56 md:h-72 shrink-0">
        {product.bgImages}
      ...
      <div className="w-full md:w-2/3 flex flex-col">
        <h3 className="text-xl sm:text-2xl md:text-3xl font-bold text-gray-900 mb-1.5">{product.name}</h3>
        
        {/* Rating */}
        <div className="flex items-center gap-2 mb-4 sm:mb-6">
          <div className="flex items-center gap-0.5">
            {[1,2,3,4].map(i => (
              <svg key={i} className="w-4 h-4 sm:w-5 sm:h-5" viewBox="0 0 24 24" fill="#fbbf24" xmlns="http://www.w3.org/2000/svg">
          ...
          <span className="text-gray-500 font-medium text-sm sm:text-base">({product.reviewCount} تقييم)</span>
        </div>

        <p className="text-gray-800 font-bold mb-2 sm:mb-3 text-sm sm:text-lg">حدد الكمية:</p>
          ...
          {product.options.map((opt) => {
            ...
            return (
              <div 
                ...
                className={`relative flex-1 cursor-pointer rounded-xl sm:rounded-2xl border-2 p-3 sm:p-4 transition-all flex flex-row sm:flex-col items-center sm:items-start justify-between sm:justify-start ${
                  isSelected ? 'border-[#5b3146] bg-[#5b3146]/5' : 'border-gray-200 hover:border-gray-300'
                } ${opt.bestValue ? 'mt-3 sm:mt-0 sm:pt-7 shadow-md' : 'shadow-sm'}`}
                style={opt.bestValue ? { borderColor: isSelected ? '#5b3146' : '#5b3146' } : {}}
              >
                {opt.bestValue && (
                  <span className="absolute -top-3.5 left-1/2 -translate-x-1/2 bg-[#5b3146] text-white text-[10px] sm:text-xs font-bold py-0.5 px-3 sm:py-1 sm:px-4 rounded-full whitespace-nowrap shadow-md">
                    الأفضل قيمةً
                  </span>
                )}
                
                <div className="flex items-center gap-2 sm:w-full sm:justify-between sm:mb-2">
                  <div className={`w-4 h-4 sm:w-5 sm:h-5 rounded-full border-2 flex items-center justify-center shrink-0 ${isSelected ? 'border-[#5b3146]' : 'border-gray-300'}`}>
                    {isSelected && <div className="w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full bg-[#5b3146]"></div>}
                  </div>
                  <span className={`font-bold text-sm sm:text-lg ${isSelected ? 'text-[#5b3146]' : 'text-gray-700'}`}>{opt.label}</span>
                </div>
                
                <div className="flex flex-col items-end sm:items-start text-right sm:text-left">
                   <div className="flex items-center sm:items-end gap-1.5 sm:gap-2 mb-0.5 sm:mb-1 flex-row-reverse sm:flex-row">
                     {opt.oldPrice && <span className="text-gray-400 line-through text-[11px] sm:text-sm">{opt.oldPrice}</span>}
                     <span className={`font-bold text-base sm:text-2xl ${isSelected ? 'text-[#5b3146]' : 'text-gray-900'}`}>{opt.price} <span className="text-[10px] sm:text-sm font-normal">EGP</span></span>
                   </div>
                   {opt.freeShipping ? (
                     <div className="text-[#25D366] font-bold text-[10px] sm:text-sm bg-[#25D366]/10 px-1.5 sm:px-2 py-0.5 sm:py-1 rounded w-fit">
                       + الشحن المجاني!
                     </div>
                   ) : (
                     <div className="text-gray-500 text-[10px] sm:text-xs leading-tight sm:leading-snug">
                       تطبق رسوم شحن
                     </div>
                   )}
                </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 mt-auto">
          <button 
            ...
            className="flex-1 bg-[#5b3146] hover:bg-[#452434] text-white font-bold py-3 sm:py-3.5 px-4 rounded-xl flex justify-center items-center gap-2 transition-colors shadow-md text-base sm:text-lg"
          >
          ...
          <button 
            ...
            className="flex-1 bg-[#25D366] hover:bg-[#1ebd5a] text-white font-bold py-3 sm:py-3.5 px-4 rounded-xl flex justify-center items-center gap-2 transition-colors shadow-md text-base sm:text-lg"
          >
  ...
  return (
    <section className="py-20 bg-[#faf8f9] font-['Cairo']" id="offers-section">
      ...
      <div className="max-w-6xl mx-auto text-center px-4 mb-12">
        ...
        <p className="text-gray-600 mb-8 font-semibold text-lg" dir="rtl">كلما زادت الكمية، زادت التوفير! اطلب أكثر واحصل على أفضل سعر مع شحن مجاني.</p>
      </div>
